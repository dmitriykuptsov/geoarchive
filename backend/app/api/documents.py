...
def download_document(
    ...
    if has_access is None and not is_global_admin(db=db, user=current_user):

        raise HTTPException(
            status_code=(status.HTTP_403_FORBIDDEN),
            detail=("Access to this deposit " "is required"),
        )
...
def delete_document(
    ...
    if has_admin_access is None and not is_global_admin(db=db, user=current_user):
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=("ADMIN access to this deposit " "is required"),
        )
...
def process_document_endpoint(
    ...
    if document.status in (DocumentStatus.PROCESSED):
        db.execute(
